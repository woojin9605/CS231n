...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (2) Use a word embedding layer to transform the words in captions_in     #
        #     from indices to vectors, giving an array of shape (N, T, W).         #
        x, cache_we = word_embedding_forward(captions_in, W_embed) #ground truth captions in word vector
    ...
    def sample(self, features, max_length=30):
        ...
        W = W_embed.shape[1]
        x = np.ones((N,W)) * W_embed[self._start] #initialize all first data to <start>
        ...
        for i in range(max_length):
          if self.cell_type == 'rnn':
            ...
            c0 = next_c
          out = next_h.dot(W_vocab) + b_vocab # vocab scores
          max_indices = np.argmax(out, axis = 1) #pick each maximum scores
          captions[:, i] = max_indices
          x = W_embed[max_indices] # into next input x data
          h0 = next_h
