...
def content_loss(content_weight, content_current, content_original):
    ...
    F_l = content_current.view(1, C_l, H_l*W_l)
    P_l = content_original.view(1, C_l, H_l*W_l)
    L_c = content_weight * torch.sum((F_l - P_l)**2)
...
def gram_matrix(features, normalize=True):
    ...
    F_l = features.view(N,C,H*W)
    G_l = F_l.matmul(F_l.permute(0,2,1)) # permute is similar with transpose.
    # permute change position of dim
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    L_s = torch.tensor(0.).type(dtype)

    for i in (range(len(style_layers))): # style_targets: List of the same length as style_layers,
      G_l = gram_matrix(feats[style_layers[i]], normalize=True)
      L_s += style_weights[i] * torch.sum((G_l - style_targets[i])**2)
    return L_s
